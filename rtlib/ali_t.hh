...
enum class ali_t:std::uint8_t {N_BASE, A_BASE, C_BASE, G_BASE, U_BASE, GAP_BASE, SEPARATOR_BASE, MOTIF_EXTENDER, EMPTY, MATCH, MISMATCH, INDEL, INDEL_EXTENDER };
static char ALI_BASE_CHARS[to_integral(ali_t::INDEL_EXTENDER)+1] = {'N','A','C','G','U','_','+','~',' ','|','*', '=','-'};

// Convert an ali_t base into the corresponding character it is meant to represent.
inline char ali_base_to_char(int a) {
    if (a < to_integral(ali_t::INDEL_EXTENDER)+1) {
...
inline char char_to_ali_base(char a) {
  char c = upper_case(a);  
  for (int i = 0; i <= to_integral(ali_t::INDEL_EXTENDER); ++i) {
